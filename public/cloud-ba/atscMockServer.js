...
const queryAlerting = require('./aeatMock/mock.query.alerting.js');
const startInjectingAlerts = require('./aeatMock/mock.subscribe.notification.js');

const PORT = 8889;
let wsShimServer;
let receiveAeatNotifications = false;
...
const methodToResponseMap = {
  ...
  'org.atsc.query.alerting': queryAlerting(),
  ...
  'org.atsc.query.baseURI':{ jsonrpc: '2.0', id: 7, result: {baseURI:null} }
};

const wsAtscCmdServerConnectionHandler = ws => {
  ws.on('close', e => console.log('BA ws client disconnected atscCmd', e));
  ...
  ws.on('message', incomingMessageStr => {
    ...
    if (incomingMessage.method === 'org.atsc.subscribe' && incomingMessage.params.msgType.includes('alertingChange')) {
      startInjectingAlerts(ws);
    }
...
const wsShimServerConnectionHandler = ws => {
  wsShimServer = ws;
  ws.on('close', e => console.log('BA ws client disconnected shim', e));
};
